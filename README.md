...
At the current stage, we provide the models supporting CoreNLP
3.6.0, 4.0.0, 4.1.0, 4.2.0, 4.2.1, 4.2.2, 4.3.0, 4.3.1, 4.3.2, and 4.4.0.
...
For example, the model for CoreNLP 4.4.0 is in
...
as do 4.3.0, 4.3.1, and 4.3.2.)*

Moreover, we provide shortcuts (Go's type aliases) in package `model/pb`
for different versions of `Document`, `Sentence`, and `Token`.
They are named in the following form:

* `DocXYZ` is a shortcut for the `Document` struct corresponding to CoreNLP X.Y.Z.
* `SentenceXYZ` is a shortcut for the `Sentence` struct corresponding to CoreNLP X.Y.Z.
* `TokenXYZ` is a shortcut for the `Token` struct corresponding to CoreNLP X.Y.Z.

You can also define type aliases in your code to avoid
facing the dizzying type names every time:

```go
import "github.com/donyori/gocorenlp/model/pb"

type Doc = pb.Doc440
type Sentence = pb.Sentence440
type Token = pb.Token440

// Then use your type aliases in your code.
```
